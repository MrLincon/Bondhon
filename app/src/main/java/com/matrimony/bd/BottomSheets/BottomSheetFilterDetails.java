...
public class BottomSheetFilterDetails extends BottomSheetDialogFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        try {
            db.collection("userDetails").document(userID).collection("acceptedRequest").document(ItemID).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                @Override
                public void onSuccess(DocumentSnapshot documentSnapshot) {
                    if (documentSnapshot.exists()) {
                        requestText.setText("একসেপটেড");
                    }
                }
            });
        }catch (Exception e){
